...
<div class="middle_title">
	<h2 class="auto-style7">모집공고 게시판</h2>
</div>
...
<div class = "main_table" align="center">	
	...
	<div class="select"  align="right" style="display: inline-block;" >
			...
			</select> &nbsp;<input class="button" type="submit" value="검색"><br/><br/>
...
<thead>
  <tr>
    <th  style="width: 81px; height: 15px;" class="auto-style1">번호</th>
    <th  style="width: 615px; height: 15px;" class="auto-style1">공고제목</th>
    <th  style="width: 89px; height: 15px;" class="auto-style1">조회수</th>
    <th  style="width: 133px; height: 15px;" class="auto-style1">상태</th>
    <th  style="width: 178px; height: 15px;" class="auto-style1">등록일</th>
  </tr>
  </thead>
  <tbody class="contents">
		...
		<tr>
			<td class="auto-style3" colspan="5" align="center"> 게시글이 없습니다. </td>
		</tr>
		</tbody>
		<%
		...
		<tr>
			...
			<!-- <input type ="button" name="district_select" id="district_select" value="상세 지역 선택" onclick="city_check();">	-->
			...
			<%
				...
				if(count >0 && fCount == 0 && (("N").equals(rRecruitment_location_city_opt)) && (("N").equals(rRecruitment_location_district_opt)) && (("N").equals(rRecruitment_necessary_job_opt))) {
					...
					if(startPage > pageBlock) {
						%>
						<a href="worker_recruitment_view?pageNum=<%=startPage - 10 %>">[이전]</a>
					<% 
					}
					...
					<% }
						...
						if(endPage < pageCount){
							%>
							<a href="worker_recruitment_view?pageNum=<%=startPage + 10%>">[다음]</a>
						<% 
					...
					<a href="worker_recruitment_view?pageNum=<%=startPage - 10%>&rRecruitment_necessary_job_opt=<%=rRecruitment_necessary_job_opt%>&rRecruitment_location_city_opt=<%=rRecruitment_location_city_opt%>&rRecruitment_location_district_opt=<%=rRecruitment_location_district_opt%>">[이전]</a>
					...
					if(endPage < pageCount){
			%>	
						<a href="worker_recruitment_view?pageNum=<%=startPage + 10%>&rRecruitment_necessary_job_opt=<%=rRecruitment_necessary_job_opt%>&rRecruitment_location_city_opt=<%=rRecruitment_location_city_opt%>&rRecruitment_location_district_opt=<%=rRecruitment_location_district_opt%>">[다음]</a>
