...
function infoConfirm() {
	...
	if(document.reg_frm.year.value.length == 0) {
		alert("생년월일은 필수사항입니다.");
		reg_frm.year.focus();
		return;
	}
	...
	if(document.reg_frm.sample4_roadAddress.value.length == 0 && document.reg_frm.sample4_jibunAddress.value.length == 0 ) {
		alert("주소는 필수사항입니다.");
		reg_frm.wEmail.focus();
		return;
	}
	
	if(document.reg_frm.wJob_s.value == 0) {
		alert("직업을 선택해주세요");
		return;
	}
